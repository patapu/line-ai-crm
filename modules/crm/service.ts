...
export async function createLead(input: z.infer<typeof LeadCreate>, actor: Actor, db?: Db): Promise<Lead> {
  ...
  // 9,999,999,999.99): protects against overflow for any caller that
  // invokes the service directly without parsing LeadCreate or LeadUpdate
  // first. Compare the value rounded to cents (not the raw float) against
  // 1e12 cents: a value like 9_999_999_999.999 is itself just under 1e10,
  // but rounds to 10000000000.00 once stored in a Decimal(12,2) column,
  // which overflows.
