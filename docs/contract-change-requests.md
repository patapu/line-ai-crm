...
- Status: APPROVED
- Decision: 2026-09-17, approved by Pakorn ("approve", meaning CR-4 is approved). Implemented in PR #PR_NUMBER (https://github.com/patapu/line-ai-crm/pull/PR_NUMBER) on branch `lane-a-cr4-messagebubble-mount`: `Timeline` renders message items with `MessageBubble` alone (props `{ message, canRetry }`, `canRetry` from the lead page's `canAct`), and activity items keep their existing card.
